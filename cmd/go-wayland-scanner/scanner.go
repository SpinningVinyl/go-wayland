...
func main() {
	...
	fmt.Fprintf(w, "// https://github.com/SpinningVinyl/go-wayland/cmd/go-wayland-scanner\n")
	...
	if protocol.Name != "wayland" {
		fmt.Fprintf(w, "import \"github.com/SpinningVinyl/go-wayland/wayland/client\"\n")
		fmt.Fprintf(w, "import xdg_shell \"github.com/SpinningVinyl/go-wayland/wayland/stable/xdg-shell\"\n")
		fmt.Fprintf(w, "import tablet \"github.com/SpinningVinyl/go-wayland/wayland/stable/tablet-v2\"\n")
		fmt.Fprintf(w, "import ext_foreign_toplevel_list \"github.com/SpinningVinyl/go-wayland/wayland/staging/ext-foreign-toplevel-list-v1\"\n")
		fmt.Fprintf(w, "import ext_image_capture_source \"github.com/SpinningVinyl/go-wayland/wayland/staging/ext-image-capture-source-v1\"\n")
	}
...
func getInputFile(file string) (io.ReadCloser, error) {
	if strings.HasPrefix(file, "http") {
		...
		if resp.StatusCode != http.StatusOK {
			resp.Body.Close()
			return nil, fmt.Errorf("GET %s: %s", file, resp.Status)
		}
...
func writeRequest(w io.Writer, ifaceName string, opcode int, r Request) {
	...
	for _, arg := range r.Args {
		...
		argIface := interfaceGoType(arg.Interface)
	...
	for _, arg := range r.Args {
		...
		case "fd":
			fdArgs = append(fdArgs, argNameLower)
		}
	}

	if len(fdArgs) > 0 {
		fmt.Fprintf(w, "oob := unix.UnixRights(%s)\n", strings.Join(fdArgs, ", "))
...
func interfaceGoType(iface string) string {
	if isLocalInterface(iface) {
		return toCamel(iface)
	}
	switch iface {
	case "zwp_tablet_tool_v2":
		return "tablet.TabletTool"
	case "ext_foreign_toplevel_handle_v1":
		return "ext_foreign_toplevel_list.ForeignToplevelHandle"
	case "ext_image_capture_source_v1":
		return "ext_image_capture_source.ImageCaptureSource"
	}
